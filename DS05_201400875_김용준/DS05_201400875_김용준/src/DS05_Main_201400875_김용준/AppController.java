...
public class AppController {
	...
	private void doesContain() {
		...
		if (this.list().isEmpty()) {
			AppView.outputLine("! 리스트가 비어 있어서 검색 작업을 할 수 없습니다.");
		}
		else {
	...
	private void elementAt() {
		...
		if (this.list().isEmpty()) {
			...
			if(student != null) {
				AppView.outputLine("! 입력된 순서 [" + order + "]의 학생의 성적은 (" + student.score() + ") 입니다.");
			}
			else {
	...
	private void first() {
		...
		if (this.list().isFull()) {
			AppView.outputLine("! 리스트가 비어 있어서 삽입 작업을 할 수 없습니다.");
		}
		else {
			Student student = this.list().first();
			if(student != null) {
				AppView.outputLine("! [맨 앞] 학생의 성적은 (" + student.score() + ") 입니다.");
			}
		}
	}
	private void last() {
		...
		if (this.list().isFull()) {
			...
			if(student != null) {
				AppView.outputLine("! [맨 뒤] 학생의 성적은 (" + student.score() + ") 입니다.");
			}
		}
	}
	...
	private void add() {
		...
		if (this.list().isFull()) {
			AppView.outputLine("! 리스트가 꽉 차있어서 삽입 작업을 할 수 없습니다.");
		}
		else {
			int score = this.inputScore();
	...
	private void removeFrom() {
		...
		if (this.list().isEmpty()) {
			...
			if(student != null) {
				...
				AppView.outputLine("! 입력된 순서 [" + order + "]에서의 학생을 삭제하는 작업을 실패하였습니다.");
			}
		}
	}
	...
	private void removeAny() {
		...
		if (this.list().isEmpty()) {
			...
			if(student != null) {
				AppView.outputLine("! 삭제된 학생의 성적은 (" + student.score() + ") 입니다.");
			}
			else {
				AppView.outputLine("! 학생을 삭제하는 작업을 실패하였습니다.");
			}
		}
	}
		...
		if (this.list().isEmpty()) {
			...
			if(order < 0 || order >= this.list().size()) {
				...
				if(this.list().replaceAt(new Student(score), order)) {
					AppView.outputLine("! 주어진 순서 [" + order + "]에 학생의 점수가 (" + score + ")로 바뀌었습니다.");
				}
				else {
